...
- On Wayland (Ubuntu's default desktop), the dock shows the Hivemind icon for the open window instead of a generic gear.
...
- The window recovers by itself if it loses its graphics or page process, which can happen after a laptop wakes from sleep. It used to stay stuck on the loading icon until you restarted the app; your terminals keep running either way.
...
- `hive views new` also writes `PROMPT.md`: a brief holding the whole view API, the surface hole-punch, the theme variables and the sandbox rules. Write what you want at the top and hand it to your agent — it can build the view without reading this repository.
...
- Terminals no longer render wider than their tile: the pty is kept at the grid's size after every fit, instead of only when xterm happens to emit a resize. A tile whose pty had drifted — a re-attach, a fit that landed on the cols it already had, a size change applied while the tile was hidden — wrapped its output at one width while drawing at another, so long lines ran off the right edge and an agent's own status bar was drawn at the stale width.
- The wallpaper photo no longer twitches: its Ken Burns animation steps 24 times a second instead of 4, so each step moves it ~0.13px rather than ~0.8px.
