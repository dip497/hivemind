  ...
  return (
    <section className="shrink-0 max-h-[40%] overflow-y-auto border-t border-[var(--color-line)] px-2 py-1.5" aria-label="machines">
      <div className="flex items-center gap-1.5 h-6 px-1">
        ...
        <button onClick={() => openMachines({ kind: "add" })} className="ml-auto size-5 grid place-items-center rounded text-[var(--color-fg3)] hover:bg-[var(--color-bg3)] hover:text-[var(--color-fg)] cursor-pointer" title="Add a machine" aria-label="add machine">
          <Plus size={12} />
        </button>
      </div>
      {snap.machines.map((m) => {
        ...
        return (
          <button
            ...
            className="w-full flex items-center gap-2 h-7 px-1.5 rounded-md text-left hover:bg-[var(--color-bg3)] cursor-pointer"
