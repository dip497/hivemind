...
function AgentAction({ action, labels, agentSel, onAgentChange, onSpawnAgent, compact }: StandardToolbarProps & { action: ToolbarAction }) {
  ...
  return <div ref={root} className="relative flex items-center">
    ...
    {open && <div className={`hm-island absolute left-0 z-30 min-w-[140px] rounded-lg p-1 ${compact ? "bottom-full mb-1" : "top-full mt-1"}`}>
      {ready.map((agent) => <button key={agent.id} data-agent-choice={agent.id} onClick={() => { onAgentChange(agent.id); setOpen(false); trigger.current?.focus(); }}
      ...
      {missing.map((agent) => <button key={agent.id} data-agent-choice={agent.id} data-missing onClick={() => { setOpen(false); openAgentSettings(agent.id); }}
        title={`Get ${agent.label}`}
        className={`flex w-full items-center gap-2 rounded-md px-2 py-1.5 text-left text-[12px] text-[var(--color-fg3)] opacity-70 hover:bg-[var(--color-bg3)] hover:opacity-100 ${focusRing}`}>
        <agent.icon size={14} /><span className="flex-1">{agent.label}</span><Download size={12} />
      </button>)}
    </div>}
  </div>;
}
