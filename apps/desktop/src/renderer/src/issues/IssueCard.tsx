...
export function IssueCard({
  ...
  return (
    <div
      ...
      title={`open ${issue.id}`}
      className={`nodrag group rounded-md border border-[var(--color-line2)] bg-[var(--color-bg3)] p-2.5 hover:border-[var(--color-accent)] focus-visible:outline focus-visible:outline-2 focus-visible:outline-[var(--color-brand)] transition-colors ${
    ...
    >
      <div className="flex items-center gap-1.5">
        ...
        <button
          className="nodrag ml-auto inline-flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity text-[11px] px-1.5 py-0.5 rounded text-white bg-[var(--color-brand)] hover:opacity-90 cursor-pointer"
...
export function IssueRow({ issue, root, onWork }: { issue: IssueSummary; root: string; onWork: () => void }) {
  ...
  return (
    <div
      ...
      onKeyDown={onActivate(open)}
      title={`open ${issue.id}`}
      className="nodrag group flex items-center gap-2 px-2 py-1.5 rounded-md hover:bg-[var(--color-bg3)] cursor-pointer border-b border-[var(--color-line)]"
    >
      ...
      <button
        className="nodrag inline-flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity text-[10.5px] px-1.5 py-0.5 rounded text-white bg-[var(--color-brand)] hover:opacity-90 cursor-pointer shrink-0"
        aria-label={`Spawn claude to work on ${issue.id}`}
        title="spawn claude + work on this"
        onClick={(e) => {
          e.stopPropagation();
