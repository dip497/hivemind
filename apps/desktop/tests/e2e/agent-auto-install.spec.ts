...
async function launch(): Promise<void> {
  const env = {
    ...
    HIVEMIND_PLUGIN_INDEX: pathToFileURL(path.join(root, "registry", "index.json")).href,
...
test.beforeAll(() => {
  fs.cpSync(path.resolve("../../plugins"), path.join(root, "registry"), { recursive: true });
  fs.mkdirSync(bin);
...
test("an agent you remove is not added back on the next start", async () => {
  ...
  await page.evaluate(() => window.dispatchEvent(new CustomEvent("hivemind:open-settings", { detail: { page: "agent:continue" } })));
