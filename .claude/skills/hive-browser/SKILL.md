...
Make sure `agent-browser` is available (it is a small native CLI):
...
```bash
agent-browser connect "$HIVEMIND_BROWSER_CDP_PORT"   # connect once; later commands omit --cdp
agent-browser tab                                    # list tabs: shows t1/t2… + URLs
agent-browser tab t<N>                               # switch to YOUR tile's tab (match the URL)
```
