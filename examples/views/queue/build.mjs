...
// Install the RESULT: `hive views install examples/views/queue/dist`.
...
await build({
  ...
  outfile: path.join(dist, "queue.js"), logLevel: "warning",
});
for (const f of ["index.html", "queue.css"]) cpSync(path.join(here, "src", f), path.join(dist, f));
