...
import { detectClaudeState } from "./state.js";
...
export const claude: AgentProviderDef = {
  ...
  install: { url: "https://code.claude.com/docs/en/setup", command: "curl -fsSL https://claude.ai/install.sh | bash" },
  /** Anthropic's official Claude mark (simple-icons). */
  icon: {
    viewBox: "0 0 24 24",
  ...
  options: [
    // `--permission-mode bypassPermissions` is refused at startup.
    { id: "mode", label: "Permission mode", flag: "--permission-mode",
      values: { bypassPermissions: ["--dangerously-skip-permissions"] }, unattended: "bypassPermissions" },
    { id: "model", label: "Model", flag: "--model" },
    { id: "effort", label: "Effort", flag: "--effort" },
  ],
  spawnLabel: (n, { mode }) => `claude #${n}${mode && mode !== "default" ? ` · ${mode}` : ""}`,
};
