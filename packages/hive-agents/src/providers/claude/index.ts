...
export const claude: AgentProviderDef = {
  ...
  // claude's own flag vocabulary. bypassPermissions is gated behind its own
  // flag — `--permission-mode bypassPermissions` is refused at startup; the
  // canonical entry is `--dangerously-skip-permissions` (cli-reference).
  spawnArgs: ({ mode, model }) => {
    const args = mode === "bypassPermissions"
