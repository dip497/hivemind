...
const TOOLS: Tool[] = [
  ...
  // ── canvas / agent control (HCP — needs the desktop app running) ──────────
  {
    ...
    inputSchema: {
      type: "object",
      properties: {
        agent: { type: "string", description: "Agent to launch: 'claude' (default), 'codex', 'opencode', 'droid', 'pi', 'hermes', …" },
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        agent: { type: "string", description: "Runtime for every worker: 'claude' (default), 'codex', 'droid', 'pi', 'hermes', … Non-claude runtimes must be installed on the host or the worker fails to spawn (status:'error')." },
