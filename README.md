...
Drop `claude`, `codex`, `gemini`, `opencode`, `pi`, or `hermes` into real workspaces where they can
...
<img width="1920" height="1200" alt="image" src="https://github.com/user-attachments/assets/8b33c851-c4fb-456b-8231-a2ddd3583dfd" />
...
| **Pluggable agents** | `claude` / `codex` / `gemini` / `opencode` / `pi` / `hermes` each run in their own WebGL-accelerated xterm tile. Agents are an extensible registry — adding one is a single entry. Live status (idle / working / waiting / done) is detected from the command and shown on the frame header. |
